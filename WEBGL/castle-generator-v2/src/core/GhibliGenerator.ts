...
export class GhibliGenerator {
    ...
    private setupGUI() {
        ...
        this.gui = new GUI({ container: this.container, width: 320 });
        this.gui.domElement.style.position = 'absolute';
        this.gui.domElement.style.top = '20px';
        this.gui.domElement.style.right = '20px';
        this.gui.domElement.style.maxHeight = 'calc(100vh - 40px)';
        this.gui.domElement.style.overflowY = 'auto';
        this.gui.domElement.style.zIndex = '1000';

        const style = this.gui.addFolder('🎨 Art Direction');
        style.add(this.params, 'palette', Object.keys(this.palettes)).name('Color Palette').onChange(() => {
            const p = this.palettes[this.params.palette];
            if (p) {
                this.params.customWallColor = p.stone;
                this.params.customAccentColor = p.accent;
                this.params.customRoofColor = p.roof;
                this.params.customRoof2Color = p.roof2;
                this.params.customWoodColor = p.wood || '#590d22';
                this.params.customWindowColor = p.window;
                this.params.customGoldColor = p.gold || '#ffb703';
                this.params.customCrystalColor = p.crystal || '#ff758f';
                this.params.customFlagColor = p.flag || '#ff0054';
                this.params.customBgColor = p.bg || '#fce7f3';
            }
            this.gui.controllersRecursive().forEach(c => c.updateDisplay());
            this.generate();
        });
        style.add(this.params, 'seed', 1, 999999, 1).name('Layout Seed').onChange(() => this.generate());
        style.add(this.params, 'buildingStyle', ['fairytale', 'japanese', 'european_coastal']).name('Building Style').onChange(() => this.generate());
        style.add(this.params, 'lowPoly').name('Low Poly Mode').onChange(() => this.generate());
        style.close();
        
        const customColorFolder = this.gui.addFolder('🎨 Custom Color Editor');
        const onCustomColorChange = () => {
            this.params.useCustomColors = true;
            this.gui.controllersRecursive().forEach(c => c.updateDisplay());
            this.generate();
        };

        customColorFolder.add(this.params, 'useCustomColors').name('Enable Custom Colors').onChange(() => this.generate());
        customColorFolder.addColor(this.params, 'customWallColor').name('Wall Color').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customAccentColor').name('Accent Trim').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customRoofColor').name('Primary Roof').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customRoof2Color').name('Secondary Roof').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customGoldColor').name('Gold Trim & Crown').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customCrystalColor').name('Crystal Glow').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customWindowColor').name('Window Emissive').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customFlagColor').name('Flag Banners').onChange(onCustomColorChange);
        customColorFolder.addColor(this.params, 'customBgColor').name('Sky / Fog Tint').onChange(onCustomColorChange);
        customColorFolder.close();

        const arch = this.gui.addFolder('🏰 Architecture Settings');
        arch.add(this.params, 'overallScale', 0.1, 5.0, 0.1).name('Overall Size').onChange(() => this.generate());
        arch.add(this.params, 'layout', ['princess_royal', 'enchanted_spires', 'fairy_tale', 'concentric', 'linear', 'cluster']).name('Castle Layout').onChange(() => this.generate());
        arch.add(this.params, 'towerShape', ['mixed', 'round', 'square', 'octagonal']).name('Tower Shapes').onChange(() => this.generate());
        arch.add(this.params, 'roofShape', ['fairytale_spire', 'onion_dome', 'pointed', 'domed', 'flat', 'bell_curve', 'mixed']).name('Roof Shapes').onChange(() => this.generate());
        arch.add(this.params, 'complexity', 3, 12, 1).name('Complexity Level').onChange(() => this.generate());
        arch.add(this.params, 'keepHeight', 30, 100, 1).name('Keep Height').onChange(() => this.generate());
        arch.add(this.params, 'towerHeight', 20, 100, 1).name('Tower Height').onChange(() => this.generate());
        arch.add(this.params, 'flaredBases').name('Flared Bases').onChange(() => this.generate());
        arch.add(this.params, 'hasCorbels').name('Detailed Corbels').onChange(() => this.generate());
        arch.add(this.params, 'centralTowerHeight', 10, 80, 1).name('Central Tower Height').onChange(() => this.generate());
        arch.add(this.params, 'centralTowerShape', ['round', 'square', 'octagonal']).name('Central Tower Shape').onChange(() => this.generate());
        arch.add(this.params, 'fairytalePinch', 0.4, 1.0, 0.05).name('Tower Slenderness').onChange(() => this.generate());
        arch.add(this.params, 'safeSpacing', 2.0, 5.0, 0.5).name('Safe Spacing (m)').onChange(() => this.generate());
        arch.close();
        
        const layoutDetails = this.gui.addFolder('🏰 Detailed Layout & Wings');
        layoutDetails.add(this.params, 'baseWidthScale', 0.5, 2.0, 0.1).name('Keep Width Scale').onChange(() => this.generate());
        layoutDetails.add(this.params, 'baseLengthScale', 0.5, 2.0, 0.1).name('Keep Length Scale').onChange(() => this.generate());
        layoutDetails.add(this.params, 'extraSections').name('Add Extra Wings').onChange(() => this.generate());
        layoutDetails.add(this.params, 'sectionsCount', 1, 3, 1).name('Number of Wings').onChange(() => this.generate());
        layoutDetails.add(this.params, 'sectionLength', 5, 30, 1).name('Wing Length').onChange(() => this.generate());
        layoutDetails.open();

        const groundFolder = this.gui.addFolder('🌍 Ground & Extra Buildings');
        groundFolder.add(this.params, 'groundScale', 0.5, 3.0, 0.1).name('Ground Surface Scale').onChange(() => this.generate());
        groundFolder.add(this.params, 'extraBuildings').name('Add Extra Buildings').onChange(() => this.generate());
        groundFolder.add(this.params, 'buildingType', ['gatehouse', 'outpost', 'watchtower']).name('Building Type').onChange(() => this.generate());
        groundFolder.add(this.params, 'buildingCount', 1, 4, 1).name('Building Count').onChange(() => this.generate());
        groundFolder.add(this.params, 'buildingOffset', 10, 100, 5).name('Distance Offset').onChange(() => this.generate());
        groundFolder.close();

        const baseWallFolder = this.gui.addFolder('🛡️ Outer Base Wall & Courtyard');

        // "Enable Base Wall" must be added first so it appears at the top of the folder
        let wallSubCtrls: any[] = [];
        const updateWallCtrls = () => wallSubCtrls.forEach(c => this.params.hasOuterBaseWall ? c.enable() : c.disable());
        baseWallFolder.add(this.params, 'hasOuterBaseWall').name('Enable Base Wall').onChange(() => { updateWallCtrls(); this.generate(); });

        // wallTowersCtrl is referenced inside wallShapeCtrl.onChange — use let so the closure captures the variable
        let wallTowersCtrl: any;
        const wallShapeCtrl = baseWallFolder.add(this.params, 'baseWallShape', ['hexagonal', 'octagonal', 'square', 'circular']).name('Wall Shape').onChange(() => {
            // sync Polygon Sides slider to a sensible default for the selected shape preset
            if (this.params.baseWallShape === 'hexagonal') { this.params.baseWallTowers = 6; wallTowersCtrl?.setValue(6); }
            else if (this.params.baseWallShape === 'octagonal') { this.params.baseWallTowers = 8; wallTowersCtrl?.setValue(8); }
            else if (this.params.baseWallShape === 'square') { this.params.baseWallTowers = 4; wallTowersCtrl?.setValue(4); }
            this.generate();
        });
        const wallWidthCtrl  = baseWallFolder.add(this.params, 'baseWallWidth', 30, 160, 1).name('Perimeter Width (X)').onChange(() => this.generate());
        const wallDepthCtrl  = baseWallFolder.add(this.params, 'baseWallDepth', 30, 160, 1).name('Perimeter Depth (Z)').onChange(() => this.generate());
        const wallHeightCtrl = baseWallFolder.add(this.params, 'baseWallHeight', 5, 40, 1).name('Wall Height (Y)').onChange(() => this.generate());
        wallTowersCtrl       = baseWallFolder.add(this.params, 'baseWallTowers', 3, 16, 1).name('Polygon Sides / Towers').onChange(() => this.generate());
        const wallTowerRCtrl = baseWallFolder.add(this.params, 'baseWallTowerRadius', 2, 8, 0.5).name('Tower Radius').onChange(() => this.generate());
        const wallMoatCtrl   = baseWallFolder.add(this.params, 'baseHasMoat').name('Surrounding Moat').onChange(() => this.generate());
        const wallBridgeCtrl = baseWallFolder.add(this.params, 'baseBridgeLength', 10, 60, 2).name('Drawbridge Length').onChange(() => this.generate());

        wallSubCtrls = [wallShapeCtrl, wallWidthCtrl, wallDepthCtrl, wallHeightCtrl, wallTowersCtrl, wallTowerRCtrl, wallMoatCtrl, wallBridgeCtrl];
        updateWallCtrls(); // apply initial disabled state
        baseWallFolder.open();
        
        const fairytaleFolder = this.gui.addFolder('🏰 Fairytale Details');
        fairytaleFolder.add(this.params, 'echauguettes').name('Corner Echauguettes').onChange(() => this.generate());
        fairytaleFolder.add(this.params, 'lancetWindows').name('Lancet Arch Windows').onChange(() => this.generate());
        fairytaleFolder.add(this.params, 'archedBridges').name('Arched Bridges').onChange(() => this.generate());
        fairytaleFolder.add(this.params, 'bellCurveRoofs').name('Bell Curve Roofs').onChange(() => this.generate());
        fairytaleFolder.add(this.params, 'scallopedCorbels').name('Scalloped Corbels').onChange(() => this.generate());
        fairytaleFolder.close();

        const glslFolder = this.gui.addFolder('🌟 Alive Walls (GLSL)');
        glslFolder.add(this.params, 'glslFauxAO').name('Faux Ambient Occlusion').onChange(() => this.rebuildShaders());
        glslFolder.add(this.params, 'glslMagicMoss').name('Creeping Magic Moss').onChange(() => this.rebuildShaders());
        glslFolder.addColor(this.params, 'magicMossColor').name('Moss Glow Color').onChange(() => this.rebuildShaders());
        glslFolder.add(this.params, 'glslEnergyVeins').name('Energy Veins').onChange(() => this.rebuildShaders());
        glslFolder.addColor(this.params, 'energyVeinColor').name('Vein Color').onChange(() => this.rebuildShaders());
        glslFolder.add(this.params, 'veinSpeed', 0.1, 2.0, 0.1).name('Vein Pulse Speed').onChange(() => this.rebuildShaders());
        glslFolder.add(this.params, 'veinIntensity', 0.0, 1.0, 0.05).name('Vein Intensity').onChange(() => this.rebuildShaders());
        glslFolder.close();

        const features = this.gui.addFolder('✨ Magical Features');
        features.add(this.params, 'floatingOnClouds').name('☁️ Floating Clouds').onChange(() => this.generate());
        features.add(this.params, 'floatingIslands').name('🏝️ Floating Islands').onChange(() => this.generate());
        features.add(this.params, 'magicCrystals').name('🔮 Magic Crystals').onChange(() => this.generate());
        features.add(this.params, 'magicSparkles').name('✨ Starlight Sparkles').onChange(() => this.generate());
        features.add(this.params, 'magicAuraRings').name('💫 Runed Aura Rings').onChange(() => this.generate());
        features.add(this.params, 'streetLamps').name('💡 Cozy Street Lamps').onChange(() => this.generate());

        const exportFolder = this.gui.addFolder('💾 3D Export Settings');
        exportFolder.add(this.params, 'exportFormat', ['glb', 'gltf']).name('Format (GLB = Binary)');
        exportFolder.add(this.params, 'dracoCompression', ['off', 'low', 'med', 'high']).name('Draco Compression');

        this.gui.add(this.params, 'generate').name('✨ Generate Castle');
        this.gui.add(this.params, 'randomizeAll').name('🎲 Randomize All');
        this.gui.add(this.params, 'exportGLTF').name('💾 Export 3D Model (GLB/GLTF)');
        this.gui.add(this.params, 'exportHTML').name('🌐 Export Standalone HTML');
    }
