precision highp float;
            ...
            {
                //small
                float valS = noise(p * 0.5) + 0.5;
                valS = 1.0 - abs(valS - 0.5) * 2.0;
                valS = pow(valS,2.0);

                //middle
                float valM = noise(p * 0.26) + 0.5;
                valM = 1.0 - abs(valM - 0.5) * 2.0;
                valM = pow(valM,2.0);
                
                //big
                float valB = smoothstep(0.0,1.0,noise(p * 0.2) + 0.5);
            ...
            {   
                ...
                if(t>tmax){
                    ...
                    float specular2 = pow(specAngle, shininess / 2.0) * noise(pos.xz * 10000.0) * 1.0;
